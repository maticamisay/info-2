...
void calcularRecaudacionChoferes(char choferes[][10], float montos[], char codigosChoferes[], int cantidadViajes)
{
    ...
    for (int i = 0; i < MAX_CHOFERES; i++)
    {
        ...
        printf("Monto para %s: %.2f\n", codigosChoferes + i * 6, pago); // Imprime el chofer y el monto que le corresponde
...
int main()
{
    ...
    mostrarViajesCostoMayor(montos, &dia, cantidadViajes, promedio);

    // b) Calcular y mostrar el monto total recaudado por cada chofer, y el chofer con mayor recaudación
    calcularRecaudacionChoferes(choferes, montos, (char *)codigosChoferes, cantidadViajes);

    // c) Crear string con móviles que no realizaron viajes
    crearStringMovilesSinViajes(moviles, stringMovilesSinViajes);
